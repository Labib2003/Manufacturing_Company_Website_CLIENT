...
const Tool = ({ tool }) => {
  ...
  return (
    <div id="tools" className="card bg-base-100 shadow-xl">
      <figure className="px-10 pt-10 bg-white w-full aspect-square">
        <img src={image} alt="tool" className="rounded-xl" />
      </figure>
      <div className="card-body items-center text-center flex flex-col justify-between text-xl">
        <h2 className="card-title text-3xl mb-5">{name}</h2>
        <p className="mb-5 text-left leading-relaxed">{description}</p>
